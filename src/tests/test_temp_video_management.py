...
def test_list_uploaded_videos_prefers_wav_and_excludes_temp_video_folder(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(main, 'TEMP_VIDEO_DIR', temp_video)

    videos = main._list_uploaded_videos()
    folders = main._list_job_folders()
...
def test_list_uploaded_videos_hides_same_named_video_when_audio_exists(monkeypatch, tmp_path):
    ...
    same_name_video.write_text('video', encoding='utf-8')
...
def test_stage_source_media_moves_to_temp_video_and_prunes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(main, 'WORKSPACE_DIR', workspace)
    monkeypatch.setattr(main, 'TEMP_VIDEO_DIR', temp_video)
    monkeypatch.setattr(main, 'TEMP_VIDEO_KEEP_COUNT', 10)
    ...
    staged = Path(main._stage_source_media_to_temp_video(str(source)))

    assert staged.parent == temp_video
    assert staged.exists()
    assert not source.exists()
    remaining = sorted(p.name for p in temp_video.iterdir() if p.is_file())
    assert len(remaining) == 10
    assert staged.name in remaining
...
def test_resolve_job_dir_for_input_reuses_wav_parent_only(monkeypatch, tmp_path):
    ...
    assert main._resolve_job_dir_for_input(str(wav_path)) == job_dir
    resolved = main._resolve_job_dir_for_input(str(video_path))
