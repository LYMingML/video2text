...
# ---------------------------------------------------------------------------
# 生成器版本：逐步产出进度和 segments（用于 SSE 流式推送）
# ---------------------------------------------------------------------------

_BACKEND_DISPLAY_MAP: dict[str, str] = {
...
def _do_transcribe_stream(
    ...
    """提取音频并分片转录，逐步产出 segments（生成器版本，兼容旧调用接口）。"""
    from backends import get_asr_backend

    lang_code = _parse_lang_code(language)

    # 映射旧显示名到注册表类名
    backend_cls_name = _BACKEND_DISPLAY_MAP.get(backend)
    if backend_cls_name is None:
        if "VibeVoice" in backend:
            backend_cls_name = "VibeVoiceASR"
        else:
            backend_cls_name = backend

    # 根据后端选择模型参数
    if backend == "FunASR（Paraformer）":
        model_name = funasr_model.split(" ")[0].strip() if funasr_model else funasr_model
    elif "VibeVoice" in backend:
    ...
    try:
        ...
        if log_cb:
            log_cb(f"[MODEL] {asr.name}: {effective_model}")
            log_cb("[STEP] 正在加载模型...")
        yield f"⏳ 正在加载 {asr.name} 模型...", []
